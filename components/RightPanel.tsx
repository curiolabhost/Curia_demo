...
export function RightPanel({ lesson, prevLessonId, nextLessonId }: RightPanelProps) {
  ...
  const handleRun = useCallback(() => {
    ...
    const checks = activeExercise?.checks ?? []
  ...
  }, [isRunning, scheduleStatusUpdate, finalizeStatus, activeExercise])
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // Run shortcut — works everywhere except blocks if Monaco is intercepting
      ...
      if (!e.metaKey && !e.ctrlKey && !e.altKey && (e.key === 'h' || e.key === 'H')) {
        ...
        e.preventDefault()
        if (activeExercise?.hint) setHintVisible((v) => !v)
        return
      }

      // Escape: close hint or blur editor
      if (e.key === 'Escape') {
        if (hintVisible) {
          e.preventDefault()
          setHintVisible(false)
  ...
  return (
    <div className="right-panel">
      ...
      <BottomBar
        ...
        hintDisabled={!activeExercise?.hint}
        isRunning={isRunning}
